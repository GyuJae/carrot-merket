import type { NextPage } from "next";

const Stream: NextPage = () => {
  return (
    <div>
      <div className="py-4 px-2">
        <div className="w-full h-48 bg-gray-400 rounded-md shadow-sm" />
        <div className="mt-2 px-2 ">Title: Let go gyutae</div>
      </div>
      <div className="relative">
        <div className="font-medium mb-2 px-3">Chats</div>
        <div className="flex flex-col px-4 overflow-y-scroll h-80 space-y-[3.5px] bg-gray-100 py-1">
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
          <div className="flex items-center ">
            <div className="bg-pink-500 w-7 h-7 rounded-full" />
            <div className="ml-1">
              <span className="text-sm font-semibold">name</span>
              <span className="text-xs font-light ml-2">
                brabrabrabrabrabrbarb
              </span>
            </div>
          </div>
        </div>
        <div className="absoluted bottom-0 w-full px-3 py-2">
          <div className="flex w-full">
            <input
              type="text"
              className="shadow-md w-full focus:ring-orange-500 rounded-md border-gray-300 focus:border-orange-500"
            />
            <div className="p-2 bg-orange-400 text-white font-semibold rounded-r-md ml-1 cursor-pointer">
              <span>&rarr;</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Stream;
